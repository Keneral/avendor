...
unsigned int mboot_get_inhouse_img_size(char *part_name, unsigned int *size)
{
	...
	*size = 0;
	...
	/*Read mkimage header*/
	len = partition_read(part_name, (off_t)0, (uchar*)&mkimage_hdr, (size_t)sizeof(part_hdr_t));
	if (len < 0) {
		dprintf(CRITICAL, "[%s] %s partition read error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
		return -EIO;
	}
...
#define BOOT_SIG_HDR_SZ 16
	...
	*size = 0;
	...
	len = partition_read(part_name, (off_t)0, (uchar*)&boot_hdr, sizeof(boot_img_hdr));
	if (len < 0) {
		dprintf(CRITICAL, "[%s] %s boot image header read error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
		return -1;
	}
	...
	*size +=  (((boot_hdr.second_size + page_size - 1) / page_size) * page_size);
	...
	len = partition_read(part_name, (off_t)(*size), (uchar*)&boot_sig_hdr, (size_t)BOOT_SIG_HDR_SZ);
	if (len < 0) {
...
char *load_overlay_dtbo(char *part_name, unsigned int *size)
{
	...
	if (size == NULL) {
		dprintf(CRITICAL, "size is NULL in %s\n", __FUNCTION__);
		return NULL;
	}
	...
	part_hdr = (part_hdr_t*)malloc(sizeof(part_hdr_t));
	if (part_hdr == NULL) {
		dprintf(CRITICAL, "part_hdr allocate fail\n");
		return -1;
	}
	...
	ptr = malloc(part_hdr->info.dsize);
	if (ptr == NULL) {
		dprintf(CRITICAL, "oodtb overlay tree malloc failed, size: 0x%x\n", part_hdr->info.dsize);
		free(part_hdr);
		return NULL;
	}

	len = partition_read(part_name, sizeof(part_hdr_t), (uchar*)ptr, (size_t)part_hdr->info.dsize);
	if (len < 0) {
		dprintf(CRITICAL, "[%s] %s partition read error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
		free(part_hdr);
		return -1;
	...
	if ((*(ptr+0) != 0xd0) || (*(ptr+1) != 0x0d) || (*(ptr+2) != 0xfe) || (*(ptr+3) != 0xed)) {
		dprintf(CRITICAL, "invalid dtbo image format !\n");
		free(part_hdr);
		return NULL;
	}

	free(part_hdr);
	*size = fdt32_to_cpu(*(unsigned int *)(ptr+0x4));
	return ptr;
}
