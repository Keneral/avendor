...
static int mt_crash_log_dump(uint8_t *crash_log_addr, uint32_t crash_log_size, uint32_t start, uint32_t size,uint32_t aee_buf_size)
{
	...
	if (p_atf_log_ctrl->info.atf_crash_flag != ATF_LAST_LOG_MAGIC_NO)	{
		/* Dump Last + Crash to prevent flag has been broken. */
		p_atf_log_ctrl->info.atf_crash_flag = ATF_CRASH_MAGIC_NO;
	}
	/* Sanity check */
	if(!mt_crash_log_dump_sanity(p_atf_log_ctrl, start, size, aee_buf_size)){
		/* Ctl structure may be broken, copy CTRL + some RING to LAST buffer */
		memcpy((void *)crash_log_addr, (void *)(uintptr_t)mt_log_buf_start, crash_log_size);
		ret = crash_log_size;
	} else {
		/* Ctrl structure is reasonable */
		if(write_count == 0)
			return 0;
