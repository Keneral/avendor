...
{
  if (!-e $base_project_cfg) {
    print "ERROR: Can not find BASE project file $base_project_cfg\n";
  }
...
# Microtrust will support MT6752/MT6795
...
sub teeOnOffCheckOperation{
  ...
  if(($prj_tee_setting eq "no") || ($prj_tee_setting eq "yes")) {
    ...
    {
      $low_mem_str = "_low_mem"; 
    }
    $config_str = "--config=".$prj_tee_type.$arch_type."_".$final_action.$low_mem_str.".xml";
